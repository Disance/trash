...
ubuntu_EOL_OSVERSIONS="trusty xenial bionic"
...
SendMetric() {
	...
	if [ -x /usr/bin/wget ]; then
		timeout -s INT 60 wget --quiet --tries=3 --read-timeout=20 --connect-timeout=10 --no-check-certificate --post-data="hostid=${HOSTID}&runid=${RUN_ID}&${POST_DATA}" -O - "${URL}" 2>/dev/null || :
	elif [ -x /usr/bin/curl ]; then
		timeout -s INT 60 curl --silent --max-time 30 --connect-timeout 10 --insecure -d "hostid=${HOSTID}&runid=${RUN_ID}&${POST_DATA}" -o /dev/null "${URL}" || :
	else
...
if [ "#${noinstall}" != "#true" ]; then
	...
	if [ "${ISPOSTYPE}-${OSVER}" = "DEBIAN-bookworm" ] && [ "${webserver}" = "openlitespeed" ] ; then
                Error "OpenLiteSpeed web server does not support Debian 12"
		exit 1
	fi
	...
	if [ "${pkgname}" = "ispmanager-lite" ] || [ "${pkgname}" = "ispmanager-lite-common" ]; then
		...
		while [ -z ${DBTYPE} ]
		do
			echo "Choose database server for ispmanager's internal data"
			echo "1) SQLite (regular load, up to 5-10 sites, 5-10 users)"
			echo "2) MySQL (recommended when maintaining a large number of sites and users)"
			echo
			# shellcheck disable=SC2039,SC2162
			read -p "Choose database server: " d
			echo
